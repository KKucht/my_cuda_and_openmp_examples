...
__global__ void computeHistogram(unsigned long long *data, int N, unsigned long long *result) {
    ...
    __shared__ unsigned long long local_result[1024];

    local_result[localIdx] = 0;
    int s = blockDim.x;
    for (int i = 0 ;i < N; i++)
        local_result[localIdx] += data[idx + i*s];

    
    atomicAdd(result, local_result[localIdx]);
}
...
int main(int argc,char **argv) {
    ...
    //blocksingrid = ceil((double)N/threadsinblock/nr_to_add_in_block);
    blocksingrid = 1;
    nr_to_add_in_block = ceil((double)N/threadsinblock);
    printf("nr_to_add_in_block %d\n", nr_to_add_in_block);

	unsigned long long int *randomNumbers = (unsigned long long int *)calloc(blocksingrid* nr_to_add_in_block * 1024,sizeof(unsigned long long int));
	...
	cudaMalloc((void **)&randomNumbersDevice, blocksingrid* nr_to_add_in_block * 1024 * sizeof(long long int));
    cudaMalloc((void **)&resultArrayDevice,  sizeof(long long int));
    
    cudaMemcpy(randomNumbersDevice, randomNumbers, blocksingrid* nr_to_add_in_block * 1024 * sizeof(long long int), cudaMemcpyHostToDevice);
    
    // Initialize device histogram to 0
    cudaMemset(resultArrayDevice, 0, sizeof(long long int));

    computeHistogram<<<blocksingrid, threadsinblock>>>(randomNumbersDevice, nr_to_add_in_block, resultArrayDevice);
